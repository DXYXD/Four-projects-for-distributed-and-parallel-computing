...
def drawAccelGrid(drawPtr, gridSize):
    ...
    for x in range(gridSize):
        for y in range(gridSize):
            ...
            drawLine(drawPtr, p, p+acc, (100,int(255*mag/MAX_ACC),0,255) )
...
def drawBodies(drawPtr):
    for x in range(N):
        if BOUNDS.inside(POS[x]):
            ...
            r = MASS[x]*5
            t = MASS[x]*50
            drawPtr.ellipse( join(p-r,p+r) , fill = (int(trunc(t*5)),
                                                     int(trunc(t*5)),
                                                     int(trunc(t*2)),
                                                     int(trunc(t*5))))
...
def drawVels(drawPtr):
    for x in range(N):
        if BOUNDS.inside(POS[x]):
            drawLine(drawPtr, POS[x], POS[x]+VEL[x], (0,0,100,255) )

def drawAccs(drawPtr):
    for x in range(N):
        if BOUNDS.inside(POS[x]):
            drawLine(drawPtr, POS[x], POS[x]+ACC[x], (100,0,0,255) )
...
def drawBBOX(drawPtr, node):
    ...
    drawPtr.rectangle( p , outline=(0,0,100) )
...
def drawCross(drawPtr, p):
    p = convertPos(p)
    drawPtr.line( (p[0]-CROSS_SIZE,p[1],p[0]+CROSS_SIZE,p[1]), fill=(0,200,0,255) )
    drawPtr.line( (p[0],p[1]-CROSS_SIZE,p[0],p[1]+CROSS_SIZE), fill=(0,200,0,255) )
...
def task_update():
    ...
    im = Image.new('RGBA', IMAGE_SIZE, (0,0,0,255))
